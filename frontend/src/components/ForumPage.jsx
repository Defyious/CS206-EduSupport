...
const educationLevels = [
  ...
  const educationLevelMap = {
    PRI_1: "Primary 1", PRI_2: "Primary 2", // and so on for all levels...
    SEC_1: "Secondary 1", // Complete the mapping based on your enum
    JC1: "JC 1", JC2: "JC 2", OTHERS: "Others"
  };
  
  const subjectMap = {
    MATH: "Mathematics", SCIENCE: "Science", ENGLISH: "English", // Complete this mapping
    HISTORY: "History", GEOGRAPHY: "Geography", // and so on for all subjects
  };  

  // Assuming these would be fetched from the backend
const posts = [
  {
...
const ForumPage = () => {
  const [allPosts, setAllPosts] = useState([...posts]); // Start with hardcoded posts
  const [myPostsData, setMyPostsData] = useState([...myPosts]); 
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const processedPosts = data.map(post => ({
            ...post,
            eduLevel: educationLevelMap[post.eduLevel] || "Unknown Level",
            subject: subjectMap[post.subject] || "Unknown Subject",
            // Assuming `image` is a base64 encoded string; if not, you'd need to convert
            imageUrl: post.image ? `data:image/jpeg;base64,${post.image}` : undefined
          }));
          setAllPosts(prevPosts => [...prevPosts, ...processedPosts]); // Combine with hardcoded posts
        } else {
    ...
    const fetchMyPosts = async () => {
      try {
        const response = await fetch(`http://localhost:8080/api/post/questions/${userID}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (response.ok) {
          ...
          setMyPostsData(prevPosts => [...prevPosts, ...processedPosts]); // Combine with hardcoded myPosts
        } else {
